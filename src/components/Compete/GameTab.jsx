...
const GameTab = ({ currentUser }) => {
  ...
  useEffect(() => {
    if (roomId) {
      ...
      axios.get(`${import.meta.env.VITE_SERVER_URL}/api/games/${roomId}`)
  ...
  useEffect(() => {
    ...
    socket.on(`invite-${currentUser.uid}`, ({ roomId }) => {
    ...
    return () => {
      ...
      socket.off(`invite-${currentUser.uid}`);
    };
  }, [currentUser.uid]);
    ...
    <div className="p-6">
      ...
      {roomId ? (
              ...
              {filteredFriends
                ...
                .map((f) => (
                  ...
                  >
                    ...
                    <span className="text-gray-800 text-sm">{f.displayName || f.email}</span>
                  </button>
                ))}
            </div>
          </div>
        ...
        <button
          ...
          onClick={async () => {
            try {
              const res = await axios.post(`${import.meta.env.VITE_SERVER_URL}/api/games/create`, {
      ...
      {incomingInvite && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-xl shadow-xl text-center space-y-4 max-w-sm">
            ...
            <p className="text-gray-700">Join room <strong>{incomingInvite}</strong>?</p>
